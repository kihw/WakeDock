...
class MonitoringService {
    ...
    private async sendReport(): Promise<void> {
        ...
        try {
            ...
            logger.error('Failed to send monitoring report', error instanceof Error ? error : new Error(String(error)));
        }
    }
     ...
     * Signaler une erreur
     */
    reportError(error: Error, context?: Record<string, any>, severity: ErrorReport['severity'] = 'medium'): string {
     ...
     */
    recordMetric(name: string, value: number, type: MonitoringMetric['type'] = 'gauge', tags?: Record<string, string>): void {
        if (Math.random() > this.config.sampleRate) return;
     ...
     */
    getAggregatedMetrics(timeWindow: number = 300000): Record<string, { avg: number; min: number; max: number; count: number }> {
     ...
     */
    get errorStats(): Readable<{ total: number; byType: Record<string, number>; bySeverity: Record<string, number> }> {
