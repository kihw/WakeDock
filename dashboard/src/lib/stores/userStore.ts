...
function createUserStore() {
    ...
    return {
        ...
        addUser: (user: User) => update(state => ({
            ...state,
            users: [...state.users, user]
        })),
        updateUser: (updatedUser: User) => update(state => ({
            ...state,
            users: state.users.map(user =>
                user.id === updatedUser.id ? updatedUser : user
            )
        })),
        removeUser: (userId: number) => update(state => ({
            ...state,
...
export const activeUsers = derived(
    userStore,
    $userStore => $userStore.users.filter(user => user.active)
);

export const adminUsers = derived(
    userStore,
    $userStore => $userStore.users.filter(user => user.role === 'admin')
);
