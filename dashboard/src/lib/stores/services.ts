...
export const services = {
    ...
    updateService: async (serviceData: UpdateServiceRequest) => {
        ...
        try {
            ...
            notifications.success('Service Updated', `${updatedService.name} has been updated successfully`);
    ...
    updateFromWebSocket: (serviceUpdate: { id: string; status: Service['status']; health_status?: Service['health_status'] }) => {
        update(state => {
            ...
            return {
                ...
                selectedService: state.selectedService?.id === serviceUpdate.id
                    ? {
