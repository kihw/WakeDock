...
class PerformanceMonitor {
    ...
    getNavigationMetrics(): Record<string, number> | null {
        if (typeof window.performance === 'undefined' || !window.performance.getEntriesByType) return null;
        ...
        return {
            ...
            'ssl-handshake': nav.secureConnectionStart > 0 ? nav.connectEnd - nav.secureConnectionStart : 0,
    ...
    getResourceMetrics(): PerformanceResourceTiming[] {
        if (typeof window.performance === 'undefined' || !window.performance.getEntriesByType) return [];
