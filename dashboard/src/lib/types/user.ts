...
export function hasPermission(user: User, permission: string): boolean {
    return user.permissions?.includes(permission) || user.roles?.includes('admin') || user.role === 'admin';
}
...
export function canManageUsers(user: User): boolean {
    return hasPermission(user, UserPermissions.USER_CREATE) ||
...
export function canManageServices(user: User): boolean {
    return hasPermission(user, UserPermissions.SERVICE_CREATE) ||
