...
<script lang="ts">
    ...
    const handleDelete = async () => {
        ...
        if (confirmed) {
            try {
                ...
                ui.showSuccess(
                    "Service deleted",
                    `Service "${service.name}" has been deleted.`,
                );
                goto("/services");
            } catch (error) {
                ui.showError(
    ...
    const loadLogs = async () => {
        ...
        try {
            logs = await api.getServiceLogs(service.id, logLines);
            showLogs = true;
        } catch (error) {
...
<svelte:head>
    <title
        >{service ? `${service.name} - ` : ""}Service Details - WakeDock</title
    >
</svelte:head>
                                        ...
                                        <div class="bg-gray-50 rounded-md p-3">
                                            <dl class="space-y-2">
                                                {#each Object.entries(service.environment) as [key, value]}
                                                    <div class="flex">
                                                        <dt
                                                            class="text-sm font-medium text-gray-500 w-1/3"
                                                        >
                                                            {key}
                                                        </dt>
                                                        <dd
                                                            class="text-sm text-gray-900 w-2/3 font-mono"
                                                        >
                                                            {value}
                                                        </dd>
                                                    </div>
